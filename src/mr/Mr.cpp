...
TS::Type resolve_type(int id, TS::TypeContext &context) {
  ...
  return std::visit(
      overloaded{[&](TS::VarType &val) -> TS::Type { throw std::exception(); },
...
Stmt MrBuilder::lower_item(AST::Item& item) {
    return std::visit(overloaded{
        [&](AST::FnItem& val) -> Stmt {
            ...
            Pat fn_pat{IdentPat{
                AST::BindingMode::make(),  // Default binding mode
                item.ident.symbol,
                std::nullopt
            }};
...
Id MrBuilder::make_expr(AST::Expr& expr) {
    ...
    result.type = std::move(resolve_type(expr.id, *type_context));
        ...
        result.kind = std::visit(overloaded{
        ...
        [&](AST::BreakExpr& val) -> Expr::ExprKind {
            return BreakExpr{
                val.expr ? std::make_optional(make_expr(*val.expr.value())) : std::nullopt
            };
        },
...
Id MrBuilder::make_param(AST::Param &param) {
   Param p;
   return mr.insert_param(Param {
    .pat = make_pat(*param.pat),
